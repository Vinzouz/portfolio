...
export function ProjectModal({ isOpen, onClose, project }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <div className="glass-cardProjectModal w-[95vw] md:w-[50vw] h-[90vh] md:h-auto overflow-hidden flex flex-col relative mx-4 rounded-lg pt-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-2 text-sm mb-4">
            <div className="flex items-center gap-2">
              <Layers className="text-indigo-600 content-center" />
              <span className="font-semibold text-lg">{t("technical stack")} :</span>
            </div>
            <div className="flex flex-wrap gap-2">
              {project.technologies.map((tech, index) => (
                <span
                  key={index}
                  className="px-3 py-1 text-sm font-medium rounded-full text-white hover:scale-105 transition-all"
                  style={{
